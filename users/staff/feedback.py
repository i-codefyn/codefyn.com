...
class FeedbackList(StaffRequiredMixin, TestMixinUserEmail, ListView):
    """Views By date"""

    login_url = reverse_lazy("account_login")
    form_class = DateForm
    model = FeedBack
    template_name = "staff/dashboard/feedback/list." + app_settings.TEMPLATE_EXTENSION
    initial = {"key": "value"}
    ...
    def form_valid(self, form):
        ...
        context = {
            "feedback": self.model.objects.all().filter(created_at__gte=fromdate),
            "form": form,
            "app_data": Sites.objects.all(),
            "pagename": "Feedback list",
            "app_data": Sites.objects.all(),
        ...
        if enddate:
            context = {
                "feedback": self.model.objects.all().filter(created_at__lte=enddate),
                "form": form,
                "pagename": "Feedback List",
                "app_data": Sites.objects.all(),
                "page_title": app_settings.PAGE_TITLE,
